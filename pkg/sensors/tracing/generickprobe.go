...
func addGenericKprobeSensors(kprobes []v1alpha1.KProbeSpec, btfBaseFile string) (*sensors.Sensor, error) {
	...
	for i := range kprobes {
		...
		retProbe := program.MapBuilderPin("retprobe_map", fmt.Sprintf("%s-retprobe", pinFile), load)
		...
		if setRetprobe {
			...
			retProbe := program.MapBuilderPin("retprobe_map", fmt.Sprintf("%s-retprobe", pinFile), loadret)
			maps = append(maps, retProbe)
		}
...
func loadGenericKprobeSensor(bpfDir, mapDir string, load *program.Program, version, verbose int) error {
	...
	if !load.RetProbe {
		load.Selectors = []program.MapData{{"filter_map", gk.loadArgs.filters[:]}}
	}
	binary.Write(&bin_buf, binary.LittleEndian, gk.loadArgs.config)
	load.Configs = []program.MapData{{"config_map", bin_buf.Bytes()[:]}}
